...
class KGDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.model in ["t5", "bart"]:
            source = self.tokenizer.batch_encode_plus([text], padding='max_length', max_length=self.source_len,
                                                      return_tensors='pt', truncation=True)
            target = self.tokenizer.batch_encode_plus([ctext], padding='max_length', max_length=self.target_len,
                                                      return_tensors='pt', truncation=True)

        elif self.model == "gpt2":
            if self.is_eval:
                if not self.batch_gen:
                    source = self.tokenizer.batch_encode_plus([text], padding=False, max_length=self.source_len,
                                                              return_tensors='pt', truncation=True)
                    target = self.tokenizer.batch_encode_plus([ctext], padding=False, max_length=self.target_len,
                                                              return_tensors='pt', truncation=True)
                else:
                ...
                target = source
                head_relation = self.tokenizer.batch_encode_plus([text], padding='max_length',
                                                                 max_length=self.source_len + self.target_len,
                                                                 return_tensors='pt', truncation=True)
                # for computing loss on tail, 
                # label tokens from head and relation by -100, so GPT2LMHealModel will not compute loss on that
                labels = source['input_ids'] * (1 - head_relation['attention_mask']) - 100 * head_relation[
                    'attention_mask']
        else:
