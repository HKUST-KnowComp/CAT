...
def main():
    ...
    config = wandb.config
    config.GPU = int(os.environ.get("GPU", 0))
    ...
    config.SEED = int(os.environ.get("SEED", 621))
    config.IN_LEN = int(os.environ.get("IN_LEN", 22))  # 40 for Stage2 NL
    config.OUT_LEN = int(os.environ.get("OUT_LEN", 36))  # 52 for Stage2 NL
    ...
    config.PRED_FILE = str(os.environ.get("PRED_FILE", "PRED_DATA_PATH"))
    ...
    config.USE_NL_RELATION = bool(os.environ.get('USE_NL_RELATION',
                                                 "False") == "True")  # whether to use natural language descriptions for the relations.
    ...
    val_dataset = pd.read_csv(os.environ.get('DEV_DATA_PATH', "DEV_DATA_PATH_GOES_HERE"),
                              sep="\t", keep_default_na=False)
    if DEBUG:
    ...
    test_dataset = pd.read_csv(os.environ.get('TEST_DATA_PATH', "TRAINING_DATA_PATH_GOES_HERE"),
                               sep="\t", keep_default_na=False)
    if DEBUG:
    ...
    val_set = KGDataset(val_dataset, tokenizer, config.IN_LEN, config.OUT_LEN - config.IN_LEN, model="gpt2",
                        is_eval=False)
    # is_eval controls whether the dataset use batches.
    # Even is_eval is false, we use batches to calculate validation loss 
    test_set = KGDataset(test_dataset, tokenizer, config.IN_LEN, config.OUT_LEN - config.IN_LEN, model="gpt2",
                         is_eval=True)
    ...
    if config.DO_PRED:
        ...
        if not config.ZERO_SHOT:
            ...
            pred_dataset["head_event"] = pred_dataset["head_event"].str.replace("PersonX", "Anderson").str.replace(
                "PersonY", "Brown").str.replace("PersonZ", "Carter")
            pred_dataset["head_event"] = pred_dataset["head_event"].str.replace('[', "\"").str.replace(']', "\"")
            pred_dataset["head_event"] = pred_dataset["head_event"] + ' ' + \
                                         pd.Series(map(lambda r: CS_RELATIONS_2NL[r], pred_dataset["relation"]))
        ...
        if not all([t == ' [EOS]' for t in pred_dataset_full['tail_event']]):
            get_eval_results(
                os.path.join(config.OUT_DIR, os.path.basename(config.PRED_FILE) + "_pred_generations.jsonl"),
